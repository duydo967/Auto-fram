--[[
                            Duydo HUB PROJECT
            This was made by Duydo Team ( duydodzhub )
            Key System Service by Duydo Team
            Compiled by: Duydo
            Copyright © 2022-2026 Duydo Team - All Rights Reserved.
]]--
local Directory = "https://raw.githubusercontent.com/flazhy/QuantumOnyx/refs/heads/main/Games"
local Scripts = {
    Free = {
        [994732206] = Directory .. "/../QuantumOnyx.lua",
        [9186719164] = Directory .. "/SailorPiece.lua",
        [8191429227] = Directory .. "/CutTrees.lua"
    }
}
local FOLDER = "Duydo Hub"
local KEY_FILE = FOLDER .. "/key.json"
local VALID_KEY = "duydodzfreehub"
...
local function LoadSavedKey()
    if isfolder and isfile and readfile and isfolder(FOLDER) and isfile(KEY_FILE) then
...
local function LoadScript(tier, key)
    ...
    if not url then
        warn("[Duydo] No " .. tier .. " script for GameId: " .. tostring(gameId))
        return
    end
    local ok, err = pcall(function() loadstring(game:HttpGet(url))() end)
    if not ok then warn("[Duydo] Error: " .. tostring(err)) end
end
local function ShowKeyUI()
    ...
    local UpdateLog = {
        { version = "v1.0", date = "2026", notes = "được mod lại by duydodz" },
    }
    ...
    PasteBtn.MouseButton1Click:Connect(function()
        if getclipboard then pcall(function() KeyInput.Text = getclipboard() or "" end) end
    end)
    ...
    keyStr = tostring(keyStr or ""):match("^%s*(.-)%s*$") or ""
    ...
    submitting = true
    SetStatus("Checking key...", Color3.fromRGB(175, 150, 255))

    task.spawn(function()
        task.wait(0.2)
        submitting = false

        if keyStr == VALID_KEY then
            isPremium = true
            resultKey = keyStr
            SaveKey(keyStr)
            if getgenv then getgenv().script_key = keyStr end
            ...
            resultKey = nil
            ClearKey()
            SetStatus("Free mode", Color3.fromRGB(255, 200, 80))
        end
    end)
end
    local BtnY = 202
...
local function AuthenticateAndLoad()
    ...
    if tostring(SavedKey or ""):match("^%s*(.-)%s*$") == VALID_KEY then
        if getgenv then getgenv().script_key = SavedKey end
        LoadScript("Free", SavedKey)
        return
    end
    ...
    task.spawn(function()
        ...
        if premium and key == VALID_KEY then
            SaveKey(key)
